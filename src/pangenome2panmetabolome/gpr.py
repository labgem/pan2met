...
def enzymes_of_reaction(pgdb, reaction: str) -> Iterable[str]:
    reaction_frame_objects = pgdb.get_frame_objects([reaction])[0]
    enzyme_reactions = reaction_frame_objects["enzymatic_reaction"]
    if enzyme_reactions is not None:
        for enzyme_reaction in enzyme_reactions:
            enzyme_reaction_frame_objects = pgdb.get_frame_objects([enzyme_reaction])[0]
            enzyme_polypeptides = enzyme_reaction_frame_objects["enzyme"]
            if enzyme_polypeptides is not None:
                yield enzyme_polypeptides
